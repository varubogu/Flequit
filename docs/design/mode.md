...
ローカルSQLiteに加え、全てのデータをプロジェクト単位、リスト単位でディレクトリ化し、タスク1件につきを1つのJSONファイル形式で保存する。
...
Webアプリケーションで動作するモード。
ローカルSQLiteファイルを作成するかわりにブラウザのWebストレージをベースファイルとして動作する。
ユーザーアカウント作成を必須とする。
ローカルファイルで動作することに加え、その差分データ用のスキーマを用意し、サーバーに送受信する。
また、ローカルファイル、クラウドストレージファイルそれぞれに固有の識別子を用意し、複数端末間の同期状況を管理する。
対象プラットフォーム：Web
ユーザーアカウント作成：任意
